...
public class SessionFailoverTest extends DevApiBaseTestCase {
     ...
     * 
     * @param hosts
     *              the hosts list, 1st has priority=100, 2nd has priority=99, and
     *              so on
     ...
     * 
     * The real socket connect timeout can be revealed only when trying to connect
     * to the unavailable remote host
     * pointed by IP address. Neither localhost IP nor domain names are working,
     * they fail much faster then the timeout
     * is reached.
     * If default 10.77.77.77:37070 doesn't work in a particular testing setup (if
    ...
    public void testConnectionTimeout() throws Exception {
        ...
        String fakeHost = (customFakeHost != null && customFakeHost.trim().length() != 0) ? customFakeHost
                : "10.77.77.77:37070";

        // TS1_1 Create a session to a Server using explicit "xdevapi.connect-timeout"
        // overriding implicit "connectTimeout".
        testConnectionTimeout_assertFailureTimeout(
                buildConnectionString(fakeHost) + "?" + makeParam(PropertyKey.xdevapiConnectTimeout, "500", true), 500,
                1500);

        // TS1_2 Create a session to a Server using explicit "xdevapi.connect-timeout"
        // overriding explicit "connectTimeout".
        testConnectionTimeout_assertFailureTimeout(
                buildConnectionString(fakeHost) + "?" + makeParam(PropertyKey.xdevapiConnectTimeout, "500", true)
                        + makeParam(PropertyKey.connectTimeout, "8000"),
                500, 1500);

        // TS1_3 Create a session to a Server using explicit "connectTimeout" overriding
        // implicit "xdevapi.connect-timeout".
        testConnectionTimeout_assertFailureTimeout(
                buildConnectionString(fakeHost) + "?" + makeParam(PropertyKey.connectTimeout, "800", true), 800, 1800);

        // TS3_1 Create a session to a remote offline host not setting the
        // "connect-timeout" parameter. The connection must timeout in ~10 seconds.
        ...
        // TS4_1 Create a session to a remote offline host setting "connect-timeout" to
        // zero (0). The connection must not timeout until cancelled.
        testConnectionTimeout_assertFailureTimeout(
                buildConnectionString(fakeHost) + "?" + makeParam(PropertyKey.xdevapiConnectTimeout, "0", true), 12000,
                600000);

        // TS6_1 Create a session using the fail over functionality passing two
        // different Server address.
        // The Server with the higher priority must be offline. The connection must
        // succeed after connect-timeout milliseconds.
        ...
        // TS6_2 Create a session using the fail over functionality passing two
        // different Server address.
        // Both Servers must be offline. The connection must time out after
        // connect-timeout * 2 milliseconds.
        testConnectionTimeout_assertFailureTimeout(
                buildConnectionString(fakeHost, fakeHost) + "?"
                        + makeParam(PropertyKey.xdevapiConnectTimeout, "500", true),
                1000, 2000);

        // TS8_1 Create a session to a Server using valid credentials passing the
        // "connect-timeout" and set it to a valid value.
        // Call the function SLEEP() and set it to 10 seconds once the connections is
        // established. No timeout exception/error must be displayed.
        long begin = System.currentTimeMillis();
        Session sess = this.fact.getSession(buildConnectionString(this.testsHost) + "?"
        ...
        // TS11_1 Set connection property xdevapi.connect-timeout=null, try to create
        // Session, check that WrongArgumentException is thrown
        // with message "The connection property 'xdevapi.connect-timeout' only accepts
        // integer values. The value 'null' can not be converted to an integer."
        assertThrows(WrongArgumentException.class,
                ...
                () -> this.fact.getSession(buildConnectionString(fakeHost, this.testsHost) + "?"
                        + makeParam(PropertyKey.xdevapiConnectTimeout, "null", true)));

        // TS11_2 Set connection property xdevapi.connect-timeout=-1, try to create
        // Session, check that WrongArgumentException is thrown with
        // message "The connection property 'xdevapi.connect-timeout' only accepts
        // integer values in the range of 0 - 2147483647, the value '-1' exceeds this
        // range."
        assertThrows(WrongArgumentException.class,
        ...
        // TS11_3 Set connection property xdevapi.connect-timeout=abc, try to create
        // Session, check that WrongArgumentException is thrown with
        // message "The connection property 'xdevapi.connect-timeout' only accepts
        // integer values. The value 'abc' can not be converted to an integer."
        assertThrows(WrongArgumentException.class,
        ...
        // TS11_4 Set connection property xdevapi.connect-timeout=, try to create
        // Session, check that WrongArgumentException is thrown with
        // message "The connection property 'xdevapi.connect-timeout' only accepts
        // integer values. The value '' can not be converted to an integer."
        assertThrows(WrongArgumentException.class,
        ...
        // TS11_5 Set connection property xdevapi.connect-timeout=12.8. Please note that
        // c/J truncates decimals w/o exception for integer parameters thus
        // the error message is not thrown against the property value. Try to connect
        // with this connection string and ensure that CJCommunicationsException
        // is thrown not earlier than 12 ms and not later than 1000 ms.
        testConnectionTimeout_assertFailureTimeout(
                buildConnectionString(fakeHost) + "?" + makeParam(PropertyKey.xdevapiConnectTimeout, "12.8", true), 12,
                1000);

        // TS12_1 Create a session to a Server giving a valid value for the
        // "connect-timeout", and use invalid credentials.
        testConnectionTimeout_assertFailureTimeout(buildConnectionStringNoUser(this.testsHost) + "?"
                + makeParam(PropertyKey.xdevapiConnectTimeout, "1000", true)
                + makeParam(PropertyKey.USER, "nosuchuser"), 0, 1000, XProtocolError.class);
    }

    private <EX extends Throwable> void testConnectionTimeout_assertFailureTimeout(String url, int expLowLimit,
