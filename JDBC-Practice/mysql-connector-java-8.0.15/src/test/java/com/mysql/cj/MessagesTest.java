...
public class MessagesTest {
        ...
        public void testLocalizedErrorMessages() throws Exception {
                ...
                assertEquals("Malformed database URL, failed to parse the main URL sections.",
                ...
                assertEquals("Failed to parse the host:port pair 'host:123'.",
                ...
                assertEquals("Connector/J cannot handle a database URL of type 'Test'.",
                ...
                assertEquals("Malformed database URL, failed to parse the port '123' as a number.",
                ...
                assertEquals("Unable to create properties transform instance 'Test' due to underlying exception: "
                                + ex.toString(),
                                Messages.getString("ConnectionString.9", new Object[] { "Test", ex.toString() }));
                assertEquals("Can't find configuration template named 'Test'",
                ...
                assertEquals("Unsupported character encoding 'Test'",
                ...
                assertEquals("\"pos\" argument can not be larger than the BLOB's length.",
                                Messages.getString("Blob.3"));
                assertEquals("\"pos\" + \"length\" arguments can not be larger than the BLOB's length.",
                                Messages.getString("Blob.4"));
                assertEquals("\"len\" argument can not be < 1.", Messages.getString("Blob.5"));
                assertEquals("\"len\" argument can not be larger than the BLOB's length.",
                ...
                assertEquals("Requested stream length of Test2 is out of range, given blob length of Test0 and starting position of Test1.",
                                Messages.getString("Blob.invalidStreamLength",
                                                new Object[] { "Test0", "Test1", "Test2" }));
                assertEquals("Position 'pos' can not be < 1 or > blob length.",
                ...
                assertEquals("Unknown type '0' in column '1' of '2' in binary-encoded result set.",
                                Messages.getString("MysqlIO.97", new Object[] { 0, 1, 2 }));

                assertEquals("No parameter named 'Test'",
                                Messages.getString("CallableStatement.3", new Object[] { "Test" }));
                assertEquals("Parameter named 'Test' is not an OUT parameter",
                                Messages.getString("CallableStatement.5", new Object[] { "Test" }));
                assertEquals("Can't find local placeholder mapping for parameter named 'Test'.",
                                Messages.getString("CallableStatement.6", new Object[] { "Test" }));
                assertEquals("Parameter number 0 is not an OUT parameter",
                                Messages.getString("CallableStatement.9", new Object[] { 0 }));
                assertEquals("Parameter index of 10 is out of range (1, 5)",
                                Messages.getString("CallableStatement.11", new Object[] { 10, 5 }));
                assertEquals("Parameter 0 is not registered as an output parameter",
                ...
                assertEquals("Can't call executeBatch() on CallableStatement with OUTPUT parameters",
                                Messages.getString("CallableStatement.25"));

                assertEquals("Illegal starting position for search, '10'",
                                Messages.getString("Clob.8", new Object[] { 10 }));

                assertEquals("Java does not support the MySQL character encoding 'Test'.",
                                Messages.getString("Connection.5", new Object[] { "Test" }));
                assertEquals(
                ...
                assertEquals("Savepoint 'Test' does not exist",
                                Messages.getString("Connection.22", new Object[] { "Test" }));
                assertEquals("Unsupported transaction isolation level 'Test'",
                ...
                assertEquals("Can't perform requested operation after getResult() has been called to write XML data",
                                Messages.getString("MysqlSQLXML.1"));

                assertEquals("Can't set IN parameter for return value of stored function call.",
                                Messages.getString("PreparedStatement.63"));
                assertEquals("'Test' is not a valid numeric or approximate numeric value",
                                Messages.getString("PreparedStatement.64", new Object[] { "Test" }));
                assertEquals("Can't set scale of 'Test1' for DECIMAL argument 'Test2'",
                                Messages.getString("PreparedStatement.65", new Object[] { "Test1", "Test2" }));
                assertEquals("No conversion from Test to Types.BOOLEAN possible.",
                ...
                assertEquals("Can't use configured regex due to underlying exception.",
                                Messages.getString("ResultSetScannerInterceptor.1"));

                assertEquals("Can't set autocommit to 'true' on an XAConnection",
                                Messages.getString("ConnectionWrapper.0"));
                assertEquals("Can't call commit() on an XAConnection associated with a global transaction",
                                Messages.getString("ConnectionWrapper.1"));
                assertEquals("Can't call rollback() on an XAConnection associated with a global transaction",
                                Messages.getString("ConnectionWrapper.2"));

                assertEquals("Illegal hour value '99' for java.sql.Time type in value 'Test'.",
                                Messages.getString("TimeUtil.0", new Object[] { 99, "Test" }));
                assertEquals("Illegal minute value '99' for java.sql.Time type in value 'Test'.",
                                Messages.getString("TimeUtil.1", new Object[] { 99, "Test" }));
                assertEquals("Illegal second value '99' for java.sql.Time type in value 'Test'.",
                ...
                assertEquals("Can not call getNCharacterStream() when field's charset isn't UTF-8",
                                Messages.getString("ResultSet.11"));
                assertEquals("Can not call getNClob() when field's charset isn't UTF-8",
                                Messages.getString("ResultSet.12"));
                assertEquals("Can not call getNString() when field's charset isn't UTF-8",
                                Messages.getString("ResultSet.14"));
                assertEquals("Internal error - conversion method doesn't support this type",
