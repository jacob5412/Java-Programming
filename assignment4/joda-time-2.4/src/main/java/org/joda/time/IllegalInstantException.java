...
/**
 * Exception thrown when attempting to create an instant or date-time that
 * cannot exist.
 * <p>
 * Classes like {@code DateTime} only store valid date-times.
 * One of the cases where validity is important is handling daylight savings
 * time (DST).
 * In many places DST is used, where the local clock moves forward by an hour in
 * spring and back by an hour in autumn/fall.
 * This means that in spring, there is a "gap" where a local time does not
 * exist.
 * <p>
 * This exception refers to this gap, and it means that your application tried
 * to create
 ...
 * When parsing, use <code>parseLocalDateTime()</code> if the string being
 * parsed has no time-zone.
 *
...
public class IllegalInstantException extends IllegalArgumentException {
    ...
    private static String createMessage(long instantLocal, String zoneId) {
        ...
        return "Illegal instant due to time zone offset transition (daylight savings time 'gap'): " + localDateTime
                + zone;
    }
